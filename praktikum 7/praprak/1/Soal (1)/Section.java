...
public class Section<T extends Product> {
    ...
    public T getProductWithNameAndType(String name, Class<? extends T> type) {
        // TODO: Lengkapi implementasi method ini
        // 1. Iterasi melalui list products
        for (T product : products) {
        
            // 2. Lakukan dua pengecekan
            boolean nameMatches = product.getName().equals(name);
            boolean typeMatches = type.isInstance(product); // Cek apakah 'product' adalah instance dari 'type'

            // 3. Jika keduanya cocok
            if (nameMatches && typeMatches) {
                System.out.println(product); // Tampilkan produk
                return product; // Kembalikan produk
            }
        }

        // 4. Jika loop selesai tanpa menemukan apa-apa
        System.out.println("Product not found");
        return null;
    }
