...
public class Member {
  // TODO: Implementasi class Member dengan exception handling
    ...
    private static final String ERROR_CODE_INVALID = "INVALID_MEMBER";

    public Member(String name, String memberId, int maxBorrowLimit) throws LibraryException {
        if (name == null || name.trim().isEmpty()) {
            throw new LibraryException("Nama member tidak boleh kosong.", ERROR_CODE_INVALID);
        }
        this.name = name.trim();

        if (memberId == null || memberId.trim().isEmpty()) {
            throw new LibraryException("Member ID tidak boleh kosong.", ERROR_CODE_INVALID);
        }
        this.memberId = memberId.trim();

        if (maxBorrowLimit <= 0) {
            throw new LibraryException("Batas peminjaman maksimal harus lebih dari 0.", ERROR_CODE_INVALID);
        }
    ...
    public void borrowBook(String bookTitle) throws LibraryException {
        if (!isActive) {
            throw new LibraryException("Anggota " + name + " tidak aktif.", "MEMBER_INACTIVE");
        }

        if (hasBorrowedBook(bookTitle)) {
            String message = String.format("Anggota %s sudah meminjam buku '%s'.", name, bookTitle);
            throw new LibraryException(message, "ALREADY_BORROWED");
        }

        if (getCurrentBorrowedCount() >= maxBorrowLimit) {
            String message = String.format("Anggota %s telah mencapai batas peminjaman (%d/%d).",
                    name, getCurrentBorrowedCount(), maxBorrowLimit);
            throw new LibraryException(message, "LIMIT_EXCEEDED");
        }
    ...
    public void returnBook(String bookTitle) throws LibraryException {
        if (!hasBorrowedBook(bookTitle)) {
            String message = String.format("Anggota %s tidak meminjam buku '%s'.", name, bookTitle);
            throw new LibraryException(message, "NOT_BORROWED");
        }
